...
public class ReactiveKafkaConfiguration {
    ...
    @Bean
    public KafkaSender<String, AppUserDTO> kafkaAppUserSender() {
        Map<String, Object> producerOps = new HashMap<>();
        producerOps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        producerOps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        producerOps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);

        SenderOptions<String, AppUserDTO> senderOptions =
                SenderOptions.<String, AppUserDTO>create(producerOps)
                        .maxInFlight(1024);
